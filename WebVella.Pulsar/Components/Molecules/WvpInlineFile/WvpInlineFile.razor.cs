...
{
	public partial class WvpInlineFile : WvpInlineBase, IAsyncDisposable
	{
		...
		private DotNetObjectReference<WvpInlineFile> _objectReference;
		...
		private async Task _onInputHandler(ChangeEventArgs e)
		{
			...
			else
			{
				...
				_value = (List<WvpFileInfo>)e.Value;
			}
			await InvokeAsync(StateHasChanged);
		}
